...
async function loadReviews(title, postId) {
  ...
  const reviewTitle = await db.posts
    .where("bookId")
    .equals(book.bookId)
    .toArray();
    ...
    reviews.forEach(async (review) => {
      ...
      let strReview = `
                <div class="row pl-2 border-bottom py-2">
                    <div class="col-sm-3">
                        <a href="profile.html?id=${usr.userId}"><img src="${
        usr.profilePicture
                    ...
                    <div class="col-sm-8">
                        <h5 class="text-primary pt-1 m-0">${usr.firstName} ${
        usr.lastName
      }</h5>
                        <small class="text-muted">${review.post.replace(
                          /(<([^>]+)>)/gi,
                          ""
                        )}</small>
                    </div>
...
`;
    ...
    <div class="row p-3">
        <div class="col-2">
            <a href="profile.html?id=${user.userId}"><img src="${
      user.profilePicture
        ...
        <div class="col-10 p-2">
            <h5 class="text-primary pt-1 m-0">${user.firstName} ${
      user.lastName
    } <small class="text-muted"><i class="fa fa-angle-right"></i> ${
      post.postType == "recommendation" ? "Recommended" : "Reviewed"
    } ${book.title} By <b>${book.author}</b></small></h5>
            <small class="text-muted p-0">${post.createdAt}</small>
        </div>
    </div>

    <div class="post-image">
        <a href="post.html?id=${post.postId}"><img src="${
      post.picture
    }" width="100%"></a>
    </div>
